...
//  		console.log(7788, currentid)
  ...
  dragSize: function(){
      if (this.isDraw && !this.cross) {
          ...
          let startx = this.currentDrawRectangle.style.left.substr(0,this.currentDrawRectangle.style.left.indexOf('px'))
          let starty = this.currentDrawRectangle.style.top.substr(0,this.currentDrawRectangle.style.top.indexOf('px'))
          let rectW = this.currentDrawRectangle.style.width.substr(0,this.currentDrawRectangle.style.width.indexOf('px'))
          let rectH = this.currentDrawRectangle.style.height.substr(0,this.currentDrawRectangle.style.height.indexOf('px'))
          startx = parseInt(startx)
          starty = parseInt(starty)
          rectW = parseInt(rectW)
...
//          		console.log(this.temprectangle)
  ...
  onMouseUp: function(){
      ...
      this.rectangleList.forEach(function(item, index){
          let curleft = this.temprectangle.startx
          let curright = this.temprectangle.endx
          let curtop = this.temprectangle.starty
          let curbottom = this.temprectangle.endy
          let curdrawH = this.temprectangle.rectH
          let curdrawW = this.temprectangle.rectW
          // let temp = 0
          // if (curleft > curright) {
          //   temp = curleft
          //   curleft = curright
          ...
          //   curbottom = temp
          // }
          let boolx = curleft<item.startx && curright > item.endx
          let booly = curtop<item.starty && curbottom > item.endy
          console.log(1112, boolx, booly, curtop, curbottom, curleft, curright, curdrawH, curdrawW)
          if (boolx || booly){
            deleteid = item.id
            // this.currentDrawRectangle.remove()
          ...
          // else {
          //   let xj = this.isOverlap(item, this.temprectangle)
          //   if (xj) {
          //     this.currentDrawRectangle.remove()
          //     this.rectangleList.splice(index, 1)
          //   }
  ...
  isOverlap: function(rect1, rect2) {
      console.log(3333333, rect1, rect2)
      var l1 = { x: rect1.startx, y: rect1.starty }
      var r1 = {x: rect1.endx + rect1.rectW, y: rect1.endy + rect1.rectH}
      var l2 = { x: rect2.startx, y: rect2.starty }
      var r2 = { x: rect2.endx + rect2.rectW, y: rect2.endy + rect2.rectH }
